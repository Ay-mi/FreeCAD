...
__title__ = "MbDFEM Command Insert New Part"
__author__ = "Ondsel"
...
class CommandInsertNewPart:
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP("MbDFEM_InsertNewPart", "New Part"),
            "Accel": "P",
            "ToolTip": QT_TRANSLATE_NOOP(
                "MbDFEM_InsertNewPart",
                "Insert a new part into the active MbDFEM. The new part's origin can be positioned in the MbDFEM.",
            ),
            "CmdType": "ForEdit",
        }
    ...
    def Activated(self):
        ...
        if not doc.FileName:
            ...
            msgBox.setText(
                translate(
                    "MbDFEM",
                    "The MbDFEM document must be saved before inserting a new part.",
                )
            )
            msgBox.setWindowTitle(translate("MbDFEM", "Save Document"))
            saveButton = msgBox.addButton(
                translate("MbDFEM", "Save"), QtWidgets.QMessageBox.AcceptRole
            )
...
class TaskMbDFEMNewPart(JointObject.TaskMbDFEMCreateJoint):
    def __init__(self):
        ...
        nameLabel = QtWidgets.QLabel(translate("MbDFEM", "Part name"))
        self.nameEdit = QtWidgets.QLineEdit()
        nameLayout.addWidget(nameLabel)
        nameLayout.addWidget(self.nameEdit)
        mainLayout.addLayout(nameLayout)
        self.nameEdit.setText(translate("MbDFEM", "Part"))

        # Add a checkbox
        self.createInNewFileCheck = QtWidgets.QCheckBox(
            translate("MbDFEM", "Create part in new file")
        )
        ...
        jointGroupBox = QtWidgets.QGroupBox(translate("MbDFEM", "Joint new part origin"))
        ...
        if newFile:
            ...
            if not Gui.getDocument(doc).saveAs():
                ...
                msgBox.setText(
                    translate(
                        "MbDFEM",
                        "If the new document is not saved the new part cannot be linked in the MbDFEM.",
                    )
                )
                msgBox.setWindowTitle(translate("MbDFEM", "Save Document"))
                saveButton = msgBox.addButton(
                ...
                cancelButton = msgBox.addButton(
                    translate("MbDFEM", "Do not Link"), QtWidgets.QMessageBox.RejectRole
                )
...
if App.GuiUp:
    Gui.addCommand("MbDFEM_InsertNewPart", CommandInsertNewPart())
