...
static const char MbDFEMKEY[] = "MbDFEM";
...
void ViewProviderMbDAssembly::setupContextMenu(QMenu* menu, QObject* receiver, const char* member)
{
    ...
    act->setChecked(isActivePart(MbDFEMKEY));
...
bool ViewProviderMbDAssembly::doubleClicked()
{
    if (isInEditMode()) {
        ...
        if (App::GetApplication()
            ...
            Gui::Command::assureWorkbench("MbDFEMWorkbench");
        }
...
bool ViewProviderMbDAssembly::canDragObjectToTarget(App::DocumentObject* obj, App::DocumentObject* target) const
{
    ...
    if (target && MbDFEMPart->hasObject(target)) {
        // If the obj stays in MbDFEM then its ok.
        return true;
    }

    // Combine the joints and groundedJoints vectors into one for simplicity.
    std::vector<App::DocumentObject*> allJoints = MbDFEMPart->getJoints();
    std::vector<App::DocumentObject*> groundedJoints = MbDFEMPart->getGroundedJoints();
...
void ViewProviderMbDAssembly::updateData(const App::Property* prop)
{
    ...
    if (prop == &obj->Group) {
        ...
        // This ensures the MbDFEM has had a chance to recompute its
        ...
        QTimer::singleShot(0, [docName, objName]() {
            ...
            auto* obj = static_cast<MbDFEM::MbDAssembly*>(pcObj);
...
bool ViewProviderMbDAssembly::setEdit(int mode)
{
    if (mode == ViewProvider::Default) {
        ...
        connectSolverUpdate = MbDFEM->signalSolverUpdate.connect([this] {
...
void ViewProviderMbDAssembly::unsetEdit(int mode)
{
    if (mode == ViewProvider::Default) {
        ...
        // Check if the view is still active before trying to deactivate the MbDFEM.
...
void ViewProviderMbDAssembly::slotActivatedVP(const Gui::ViewProviderDocumentObject* vp, const char* name)
{
    if (name && strcmp(name, MbDFEMKEY) == 0) {
        ...
        // If the new active VP is NOT this MbDFEM (meaning we lost activation or it was cleared)
...
bool ViewProviderMbDAssembly::tryMouseMove(const SbVec2s& cursorPos, Gui::View3DInventorViewer* viewer)
{
    ...
    if (canStartDragging) {
        ...
        if (enableMovement && getSelectedObjectsWithinMbDFEM()) {
    ...
    if (partMoving) {
        ...
        if (solveOnMove && dragMode != DragMode::TranslationNoSolve) {
            MbDFEMPart->doDragStep();
        }
        else {
            MbDFEMPart->redrawJointPlacements(MbDFEMPart->getJoints());
        }
    }
    return false;
}
...
void ViewProviderMbDAssembly::doubleClickedIn3dView()
{
    ...
    if (joint) {
        ...
        std::string cmd = "import MbDJointObject as JointObject\n"
                          "obj = App.getDocument('"
            + doc_name + "').getObject('" + obj_name
            + "')\n"
              "Gui.Control.showDialog(JointObject.TaskMbDFEMCreateJoint(0, obj))";
...
bool ViewProviderMbDAssembly::canDragObjectIn3d(App::DocumentObject* obj) const
{
    ...
    // Check if the selected object is a child of the MbDFEM
    if (!MbDFEMPart->hasObject(obj, true)) {
        ...
        if (linkEl) {
            ...
            if (MbDFEMPart->hasObject(linkGroup, true)) {
                return true;
            }
        }
        return false;
    ...
    if (MbDFEMPart->isPartGrounded(obj)) {
        return false;
    }
    return true;
}
...
bool ViewProviderMbDAssembly::getSelectedObjectsWithinMbDFEM(bool addPreselection, bool onlySolids)
{
    ...
    // Get the MbDFEM object for this ViewProvider
    auto* MbDFEMPart = getObject<MbDAssembly>();
    ...
    if (!moveOnlyPreselected) {
        for (auto& selObj : Gui::Selection().getSelectionEx(
            ...
            //  if you have several objects within the same MbDFEM selected.
            ...
            for (auto& subNamesStr : objsSubNames) {
                ...
                if (!obj) {
                    // In case of sub-MbDFEM, the jointgroup would trigger the dragger.
                    continue;
                }
    ...
    if (addPreselection && Gui::Selection().hasPreselection()) {
        ...
        App::DocumentObject* obj = getMovingPartFromSel(MbDFEMPart, selRoot, sub);
...
{
    // Get the MbDAssembly for context
    auto* MbDFEMPart = getObject<MbDAssembly>();
    ...
    App::DocumentObject* part = getMovingPartFromSel(MbDFEMPart, selRoot, subNamePrefix);

    if (onlySolids && MbDFEMPart->isPartConnected(part)) {
...
ViewProviderMbDAssembly::DragMode ViewProviderMbDAssembly::findDragMode()
{
    ...
    if (docsToMove.size() == 1) {
        auto* MbDFEMPart = getObject<MbDAssembly>();
        std::string pName;
        movingJoint = MbDFEMPart->getJointOfPartConnectingToGround(docsToMove[0].obj, pName);
        ...
        if (!movingJoint) {
            ...
            std::vector<MbDFEM::ObjRef> connectedParts
                = MbDFEMPart->getDownstreamParts(docsToMove[0].obj, movingJoint);
        ...
        if (jointType == JointType::Fixed) {
            ...
            auto* upPart = MbDFEMPart->getUpstreamMovingPart(docsToMove[0].obj, movingJoint, pName);
            if (!movingJoint) {
        ...
        Base::Placement asmPlc = App::GeoFeature::getGlobalPlacement(getObject<MbDAssembly>());
...
void ViewProviderMbDAssembly::tryInitMove(const SbVec2s& cursorPos, Gui::View3DInventorViewer* viewer)
{
    ...
    auto joints = MbDFEMPart->getJoints();
    for (auto* joint : joints) {
        if (!joint) {
            continue;
        }
...
void ViewProviderMbDAssembly::draggerMotionCallback(void* data, SoDragger* d)
{
    ...
    // Transform the global delta `movePlc` in case the MbDFEM is transformed.
    Base::Placement asmPlc = App::GeoFeature::getGlobalPlacement(sudoThis->getObject<MbDAssembly>());
...
void ViewProviderMbDAssembly::onSelectionChanged(const Gui::SelectionChanges& msg)
{
    ...
    bool singleMbDFEM
        = getDocument()->getDocument()->getObjectsOfType<MbDFEM::MbDAssembly>().size() == 1;
    if (!isInEditMode() && !singleMbDFEM) {
        return;
    }
    ...
    if (msg.Type == Gui::SelectionChanges::AddSelection) {
        ...
        if (enableMovement && getSelectedObjectsWithinMbDFEM(false, true)) {
            initMoveDragger();
        }
    }
    if (msg.Type == Gui::SelectionChanges::ClrSelection
...
bool ViewProviderMbDAssembly::canDelete(App::DocumentObject* objBeingDeleted) const
{
    ...
    if (res) {
        ...
        for (auto* obj : objsBeingDeleted) {
            // List its joints
            std::vector<App::DocumentObject*> joints = MbDFEMPart->getJointsOfObj(obj);
            for (auto* joint : joints) {
                if (std::ranges::find(objToDel, joint) == objToDel.end()) {
                    objToDel.push_back(joint);
                }
            }
            joints = MbDFEMPart->getJointsOfPart(obj);
            for (auto* joint : joints) {
                if (std::ranges::find(objToDel, joint) == objToDel.end()) {
                    objToDel.push_back(joint);
                }
...
void ViewProviderMbDAssembly::isolateComponents(std::set<App::DocumentObject*>& isolateSet, IsolateMode mode)
{
    ...
    std::vector<App::DocumentObject*> topLevelChildren = MbDFEM->Group.getValues();
...
void ViewProviderMbDAssembly::clearTemporaryExplosion()
{
    ...
    if (!jointPy.hasAttr("restoreMbDFEM")) {
        return;
    }

    Py::Object attr = jointPy.getAttr("restoreMbDFEM");
    if (attr.ptr() && attr.isCallable()) {
        Py::Tuple args(1);
...
inline QString objListHelper(const MbDAssembly* MbDFEM, const std::vector<std::string>& names)
{
    if (!MbDFEM) {
        return QString();
    }
...
void ViewProviderMbDAssembly::UpdateSolverInformation()
{
    ...
    int dofs = MbDFEM->getLastDoF();
    bool hasRedundancies = MbDFEM->getLastHasRedundancies();
    bool hasMalformed = MbDFEM->getLastHasMalformedConstraints();

    if (MbDFEM->isEmpty()) {
        signalSetUp(QStringLiteral("empty"), tr("Empty MbDFEM"), QString(), QString());
    }
        ...
        /*signalSetUp(QStringLiteral("conflicting_constraints"),
                    ...
                    QStringLiteral("(%1)").arg(objListHelper(MbDFEM,
           MbDFEM->getLastConflicting())));*/
        // So for now we report like follows:
        signalSetUp(
            QStringLiteral("conflicting_constraints"),
            tr("Over-constrained:") + QLatin1String(" "),
            QStringLiteral("#conflicting"),
            QStringLiteral("(%1)").arg(objListHelper(MbDFEM, MbDFEM->getLastRedundant()))
        );
    }
        ...
        signalSetUp(
            ...
            QStringLiteral("(%1)").arg(objListHelper(MbDFEM, MbDFEM->getLastMalformed()))
        );
    }
    // Currently the solver does not distinguish between conflicts and redundancies.
    ...
    MbDFEM->getLastRedundant())));
        ...
        signalSetUp(
            ...
            QStringLiteral("(%1)").arg(objListHelper(MbDFEM,
    MbDFEM->getLastPartiallyRedundant())));
    }*/
    else if (MbDFEM->getLastSolverStatus() != 0) {
        signalSetUp(
