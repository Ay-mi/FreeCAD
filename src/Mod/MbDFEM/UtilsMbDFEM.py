...
__title__ = "MbDFEM utilitary functions"
__author__ = "Ondsel"
...
def getObject(ref):
    ...
    # sub_name is "LinkOrMbDFEM1.LinkOrPart1.LinkOrBox.Edge16"
    # or           "LinkOrMbDFEM1.LinkOrPart1.LinkOrBody.pad.Edge16"
    # or           "MbDFEM.LinkOrMbDFEM1.LinkOrPart1.LinkOrBody.Local_CS.X"
    ...
    for i, obj_name in enumerate(names):
        ...
        if obj.TypeId in {"App::Part", "MbDFEM::MbDAssembly"} or isLinkGroup(obj):
            continue
...
def getGlobalPlacement(ref, targetObj=None):
    ...
    # So to get the correct global placement in case user transformed the MbDFEM,
    # or if he has nested the MbDFEM, we need to adjust it.
    # Example : Part / MbDFEM / Cylinder / Face1
    # ref is [(Cylinder, 'Face1')]
    parents = rootObj.Parents  # [(<Part object>, 'MbDFEM.Cylinder.')]
...
def isThereOneRootMbDFEM():
    if Gui.activeDocument() is None:
        return False

    for part in Gui.activeDocument().TreeRootObjects:
        if part.TypeId == "MbDFEM::MbDAssembly":
            return True
    return False


def getElementName(full_name):
    # full_name is "MbDFEM.MbDFEM1.MbDFEM2.MbDFEM3.Box.Edge16"
...
def getObjsNamesAndElement(obj_name, sub_name):
    # if obj_name = "MbDFEM" and sub_name = "MbDFEM1.MbDFEM2.MbDFEM3.Box.Edge16"
    # this will return ["MbDFEM","MbDFEM1","MbDFEM2","MbDFEM3","Box"] and "Edge16"
...
def getFullObjName(obj_name, sub_name):
    # if obj_name = "MbDFEM" and sub_name = "MbDFEM1.MbDFEM2.MbDFEM3.Box.Edge16"
    # this will return "MbDFEM.MbDFEM1.MbDFEM2.MbDFEM3.Box"
...
def getFullElementName(obj_name, sub_name):
    # if obj_name = "MbDFEM" and sub_name = "MbDFEM1.MbDFEM2.MbDFEM3.Box.Edge16"
    # this will return "MbDFEM.MbDFEM1.MbDFEM2.MbDFEM3.Box.Edge16"
...
def getBomGroup(MbDFEM):
    bom_group = None

    for obj in MbDFEM.OutList:
        if obj.TypeId == "MbDFEM::BomGroup":
            bom_group = obj
            break

    if not bom_group:
        bom_group = MbDFEM.newObject("MbDFEM::BomGroup", "Bills of Materials")
...
def getJointGroup(MbDFEM):
    ...
    for obj in MbDFEM.OutList:
        if obj.TypeId == "MbDFEM::JointGroup":
            joint_group = obj
            break

    if not joint_group:
        joint_group = MbDFEM.newObject("MbDFEM::JointGroup", "Joints")

    return joint_group


def getViewGroup(MbDFEM):
    view_group = None

    for obj in MbDFEM.OutList:
        if obj.TypeId == "MbDFEM::ViewGroup":
            view_group = obj
            break

    if not view_group:
        view_group = MbDFEM.newObject("MbDFEM::ViewGroup", "Exploded Views")

    return view_group


def getSimulationGroup(MbDFEM):
    sim_group = None

    for obj in MbDFEM.OutList:
        if obj.TypeId == "MbDFEM::SimulationGroup":
            sim_group = obj
            break

    if not sim_group:
        sim_group = MbDFEM.newObject("MbDFEM::SimulationGroup", "Simulations")
...
def isMbDFEMGrounded():
    ...
    jointGroup = getJointGroup(MbDFEM)
...
def saveMbDFEMPartsPlacements(MbDFEM):
    initialPlcs = {}
    MbDFEMParts = getMovablePartsWithin(MbDFEM)
...
def restoreMbDFEMPartsPlacements(MbDFEM, initialPlcs):
    MbDFEMParts = getMovablePartsWithin(MbDFEM)
    for part in MbDFEMParts:
...
def getComAndSize(MbDFEM):
    if MbDFEM.ViewObject is None:
    ...
    bbox = MbDFEM.ViewObject.getBoundingBox()
    if not bbox.isValid():
        return App.Vector(), 100
...
def getComponentReference(MbDFEM, root_obj, sub_string):
    """
    Takes a full selection path (root + subnames) and normalizes it
    to be rooted at the MbDFEM Component (Moving Part).

    Returns: (ComponentObject, RelativeSubString) or (None, "")
    """
    if not MbDFEM or not root_obj:
        return None, ""

    doc = MbDFEM.Document

    # 1. Reconstruct full path
    # e.g. ['Part', 'MbDFEM', 'Cylinder', 'Face1']
    names = [root_obj.Name] + sub_string.split(".")

    # 2. Find MbDFEM in path
    try:
        asm_idx = names.index(MbDFEM.Name)
    except ValueError:
        return None, ""

    # 3. Identify Component (first valid object after MbDFEM)
...
def swapElNameInSubname(sub_name, new_elName):
    # turns MbDFEM.box.edge1 into MbDFEM.box.new_elName
    names = sub_name.split(".")
