...
MbDAssemblyLink::MbDAssemblyLink()
{
    ADD_PROPERTY_TYPE(
        ...
        "If the sub-MbDFEM is set to Rigid, it will act "
...
void MbDAssemblyLink::onChanged(const App::Property* prop)
{
    ...
    if (prop == &Rigid) {
        ...
        if (!Rigid.getValue()) {
            // when the MbDFEMLink becomes flexible, we need to make sure its placement is
            ...
            if (!plc.isIdentity()) {
                ...
                // We need to apply the placement of the MbDFEM link to the children or they will
                // move.
                ...
                MbDAssembly::redrawJointPlacements(getJoints());
            }
        }
        else {
            // For the MbDFEMlink not to move to origin, we need to update its placement.
...
void MbDAssemblyLink::updateParentJoints()
{
    ...
    // Iterate joints in the immediate parent MbDFEM only (recursive=false)
    for (auto* joint : parent->getJoints(false, false)) {
        for (const char* refName : {"Reference1", "Reference2"}) {
            ...
            if (rigid) {  // Flexible -> Rigid
                if (hasObject(refObj)) {
                    // The joint currently points to a child (refObj) inside this MbDFEMLink.
...
void MbDAssemblyLink::synchronizeComponents()
{
    ...
    // We check if a component needs to be added to the MbDFEMLink
    for (auto* obj : topLevelComponents) {
        ...
        // In which case we need to add an MbDFEMLink and not a Link.
        ...
        for (auto* obj2 : MbDAssemblyLinkGroup) {
            ...
            if (subAsmLink) {
                ...
                // We consider only Links and MbDFEMLinks
                continue;
            }
    ...
    // If the MbDFEMLink is rigid, then we keep all placements synchronized.
    if (isRigid()) {
