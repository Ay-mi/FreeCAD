...
PROPERTY_SOURCE(MbDFEM::BomObject, Spreadsheet::Sheet)
...
void BomObject::generateBOM()
{
    ...
    auto* MbDFEM = getMbDFEM();
    if (MbDFEM) {
        addObjectChildrenToBom(MbDFEM->getOutList(), row, "");
    }
    else {
...
)
{
    ...
    for (auto* child : objs) {
        ...
        if (!child->isDerivedFrom<MbDAssembly>() && !child->isDerivedFrom<App::Part>()
        ...
        if ((child->isDerivedFrom<MbDAssembly>() && detailSubAssemblies.getValue())
            || (!child->isDerivedFrom<MbDAssembly>() && child->isDerivedFrom<App::Part>()
...
MbDAssembly* BomObject::getMbDFEM() const
{
    for (auto& obj : getInList()) {
        if (obj->isDerivedFrom<MbDAssembly>()) {
            return static_cast<MbDAssembly*>(obj);
        }
    }
    return nullptr;
}
...
std::string MbDFEM::BomObject::getText(size_t row, size_t col) const
{
