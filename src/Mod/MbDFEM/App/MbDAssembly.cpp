...
MbDAssembly::~MbDAssembly() = default;

PyObject* MbDAssembly::getPyObject()
{
...
App::DocumentObjectExecReturn* MbDAssembly::execute()
{
    App::DocumentObjectExecReturn* ret = App::Part::execute();

    ParameterGrp::handle hGrp = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Mod/MbDFEM"
    );
...
void MbDAssembly::onChanged(const App::Property* prop)
{
    if (prop == &Group) {
        updateSolveStatus();
    }
...
int MbDAssembly::solve(bool enableRedo)
{
...
void MbDAssembly::updateSolveStatus()
{
    ...
    auto cleanJointName = [](const std::string& rawName) -> std::string {
        // rawName is like : /OndselMbDFEM/ground_moves#Joint001
...
int MbDAssembly::generateSimulation(App::DocumentObject* sim)
{
    mbdAssembly = makeMbdAssembly();
    objectPartMap.clear();
...
std::vector<App::DocumentObject*> MbDAssembly::getMotionsFromSimulation(App::DocumentObject* sim)
{
    if (!sim) {
        return {};
    }
...
int MbDFEM::MbDAssembly::updateForFrame(size_t index)
{
    if (!mbdAssembly) {
        return -1;
    }
...
size_t MbDFEM::MbDAssembly::numberOfFrames()
{
    return mbdAssembly->numberOfFrames();
}

void MbDAssembly::preDrag(std::vector<App::DocumentObject*> dragParts)
{
    bundleFixed = true;
    solve();
    bundleFixed = false;
...
Base::Placement MbDAssembly::getMbdPlacement(std::shared_ptr<ASMTPart> mbdPart)
{
    if (!mbdPart) {
...
bool MbDAssembly::validateNewPlacements()
{
            ...
            if (it != objectPartMap.end()) {
                ...
                if (!oldPlc.isSame(newPlacement, Precision::Confusion())) {
                    Base::Console().warning(
                        "MbDFEM : Ignoring bad solve, a grounded object (%s) moved.\n",
                        obj->getFullLabel()
                    );
                    return false;
                }
...
void MbDAssembly::postDrag()
{
    mbdAssembly->runPostDrag();  // Do this after last drag
    purgeTouched();
}

void MbDAssembly::savePlacementsForUndo()
{
...
void MbDAssembly::clearUndo()
{
    previousPositions.clear();
}

void MbDAssembly::exportAsASMT(std::string fileName)
{
    mbdAssembly = makeMbdAssembly();
    objectPartMap.clear();
    fixGroundedParts();
...
void MbDAssembly::redrawJointPlacements(std::vector<App::DocumentObject*> joints)
{
...
void MbDAssembly::redrawJointPlacement(App::DocumentObject* joint)
{
    if (!joint) {
        return;
    }
...
std::shared_ptr<ASMTAssembly> MbDAssembly::makeMbdAssembly()
{
    auto MbDFEM = CREATE<ASMTAssembly>::With();
    // MbDFEM->externalSystem->freecadMbDAssembly = this;
    MbDFEM->externalSystem->freecadAssemblyObject = nullptr;
    MbDFEM->setName("OndselMbDFEM");

    ParameterGrp::handle hPgr = App::GetApplication().GetParameterGroupByPath(
        "User parameter:BaseApp/Preferences/Mod/MbDFEM"
    );

    MbDFEM->setDebug(hPgr->GetBool("LogSolverDebug", false));
    return MbDFEM;
}

App::DocumentObject* MbDAssembly::getJointOfPartConnectingToGround(
...
JointGroup* MbDAssembly::getJointGroup() const
{
    return MbDFEM::getJointGroup(this);
}

ViewGroup* MbDAssembly::getExplodedViewGroup() const
{
    App::Document* doc = getDocument();
...
std::vector<App::DocumentObject*> MbDAssembly::getJoints(bool delBadJoints, bool subJoints)
{
    ...
    if (subJoints) {
        for (auto& MbDFEM : getSubAssemblies()) {
            auto subJoints = MbDFEM->getJoints();
...
std::vector<App::DocumentObject*> MbDAssembly::getGroundedJoints()
{
    std::vector<App::DocumentObject*> joints = {};
...
std::vector<App::DocumentObject*> MbDAssembly::getJointsOfObj(App::DocumentObject* obj)
{
    if (!obj) {
        return {};
    }
...
std::vector<App::DocumentObject*> MbDAssembly::getJointsOfPart(App::DocumentObject* part)
{
    if (!part) {
        return {};
    }
...
std::unordered_set<App::DocumentObject*> MbDAssembly::fixGroundedParts()
{
    auto groundedParts = getGroundedParts();
...
void MbDAssembly::fixGroundedPart(App::DocumentObject* obj, Base::Placement& plc, std::string& name)
{
    if (!obj) {
        return;
    }
    ...
    markerName1 = "/OndselMbDFEM/" + mbdMarker1->name;
    markerName2 = "/OndselMbDFEM/" + mbdPart->name + "/" + mbdMarker2->name;
...
bool MbDAssembly::isJointConnectingPartToGround(App::DocumentObject* joint, const char* propname)
{
...
bool MbDAssembly::isJointTypeConnecting(App::DocumentObject* joint)
{
    if (!joint) {
        return false;
    }
...
bool MbDAssembly::isObjInSetOfObjRefs(App::DocumentObject* obj, const std::vector<ObjRef>& set)
{
    if (!obj) {
        return false;
    }
...
void MbDAssembly::removeUnconnectedJoints(
    std::vector<App::DocumentObject*>& joints,
...
void MbDAssembly::traverseAndMarkConnectedParts(
...
std::vector<ObjRef> MbDAssembly::getConnectedParts(
    App::DocumentObject* part,
    const std::vector<App::DocumentObject*>& joints
)
{
...
bool MbDAssembly::isPartGrounded(App::DocumentObject* obj)
{
    if (!obj) {
        return false;
    }
...
bool MbDAssembly::isPartConnected(App::DocumentObject* obj)
{
    if (!obj) {
        return false;
    }
...
void MbDAssembly::jointParts(std::vector<App::DocumentObject*> joints)
{
    for (auto* joint : joints) {
        if (!joint) {
            continue;
...
void MbDFEM::MbDAssembly::create_mbdSimulationParameters(App::DocumentObject* sim)
{
...
std::shared_ptr<ASMTJoint> MbDAssembly::makeMbdJointOfType(App::DocumentObject* joint, JointType type)
{
    switch (type) {
...
std::shared_ptr<ASMTJoint> MbDAssembly::makeMbdJointDistance(App::DocumentObject* joint)
{
...
std::vector<std::shared_ptr<MbD::ASMTJoint>> MbDAssembly::makeMbdJoint(App::DocumentObject* joint)
{
    if (!joint) {
        return {};
    }
...
std::string MbDAssembly::handleOneSideOfJoint(
    App::DocumentObject* joint,
...
)
{
    ...
    return "/OndselMbDFEM/" + mbdPart->name + "/" + markerName;
}

void MbDAssembly::getRackPinionMarkers(
    App::DocumentObject* joint,
...
)
{
    ...
    markerNameI = "/OndselMbDFEM/" + mbdPart->name + "/" + markerName;
}

int MbDAssembly::slidingPartIndex(App::DocumentObject* joint)
{
    App::DocumentObject* part1 = getMovingPartFromRef(joint, "Reference1");
    App::DocumentObject* obj1 = getObjFromJointRef(joint, "Reference1");
...
bool MbDAssembly::isMbDJointValid(App::DocumentObject* joint)
{
    ...
    if (getMbDPart(part1) == getMbDPart(part2)) {
        Base::Console().warning(
            "MbDFEM: Ignoring joint (%s) because its parts are connected by a fixed "
...
MbDAssembly::MbDPartData MbDAssembly::getMbDData(App::DocumentObject* part)
{
...
std::shared_ptr<ASMTPart> MbDAssembly::getMbDPart(App::DocumentObject* part)
{
    if (!part) {
        return nullptr;
    }
...
std::shared_ptr<ASMTPart> MbDAssembly::makeMbdPart(std::string& name, Base::Placement plc, double mass)
{
...
std::shared_ptr<ASMTMarker> MbDAssembly::makeMbdMarker(std::string& name, Base::Placement& plc)
{
...
std::vector<ObjRef> MbDAssembly::getDownstreamParts(
    App::DocumentObject* part,
    App::DocumentObject* joint
)
{
...
App::DocumentObject* MbDAssembly::getUpstreamMovingPart(
    App::DocumentObject* part,
    App::DocumentObject*& joint,
    std::string& name,
...
double MbDAssembly::getObjMass(App::DocumentObject* obj)
{
    if (!obj) {
        return 0.0;
    }
...
void MbDAssembly::setObjMasses(std::vector<std::pair<App::DocumentObject*, double>> objectMasses)
{
...
void MbDAssembly::ensureIdentityPlacements()
{
    std::vector<App::DocumentObject*> group = Group.getValues();
    for (auto* obj : group) {
        // When used in MbDFEM, link groups must have identity placements.
        if (obj->isLinkGroup()) {
...
void MbDAssembly::syncGroundedJoints()
{
        ...
        if (isReadOnly && !hasJoint) {
            ...
            try {
                ...
                std::string code = "import FreeCAD\n"
                    ...
                    + partName
                    + "')\n"
                      "    jg = UtilsMbDFEM.getJointGroup(asm)\n"
                      "    if jg:\n"
