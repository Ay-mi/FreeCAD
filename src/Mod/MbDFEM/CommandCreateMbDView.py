...
class CommandCreateView:
    ...
    def GetResources(self):
        return {
            "Pixmap": "MbDFEM_ExplodedView",
            "MenuText": QT_TRANSLATE_NOOP("MbDFEM_CreateView", "Exploded View"),
            "Accel": "E",
            "ToolTip": QT_TRANSLATE_NOOP(
                "MbDFEM_CreateView",
                "Creates an exploded view of the current MbDFEM",
            ),
            "CmdType": "ForEdit",
        }
    ...
    def IsActive(self):
        return (
            ...
            and UtilsMbDFEM.MbDFEM_has_at_least_n_parts(2)
        )
...
class ExplodedView:
    ...
    def applyMoves(self, viewObj, com=None, size=None):
        ...
        if com is None:
            com, size = UtilsMbDFEM.getComAndSize(self.getMbDFEM(viewObj))
    ...
    def explodeTemporarily(self, viewObj):
        self.initialPlcs = UtilsMbDFEM.saveMbDFEMPartsPlacements(self.getMbDFEM(viewObj))
        self.applyMoves(viewObj)
        for move in viewObj.Group:
            move.Visibility = True

    def getMbDFEM(self, viewObj):
    ...
    def saveMbDFEMAndExplode(self, viewObj):
        self.initialPlcs = UtilsMbDFEM.saveMbDFEMPartsPlacements(self.getMbDFEM(viewObj))
    ...
    def restoreMbDFEM(self, viewObj):
        if self.initialPlcs is None:
            return

        UtilsMbDFEM.restoreMbDFEMPartsPlacements(self.getMbDFEM(viewObj), self.initialPlcs)
    ...
    def _calculateExplodedPlacements(self, viewObj):
        ...
        for move in viewObj.Group:
            ...
            for sub in subs:
                ...
                if move.MoveType == "Radial":
                    obj_com, obj_size = UtilsMbDFEM.getComAndSize(obj)
    ...
    def getExplodedShape(self, viewObj):
        """
        Generates a compound shape of the exploded MbDFEM in memory
...
class ViewProviderExplodedView:
    ...
    def getIcon(self):
        return ":/icons/MbDFEM_ExplodedView.svg"
...
class ExplodedViewStep:
    ...
    def migrationScript(self, evStep):
        if hasattr(evStep, "Parts"):
            ...
            for objName, part in zip(objNames, parts):
                ...
                obj = UtilsMbDFEM.getObjectInPart(objName, part)
                rootObj, path = UtilsMbDFEM.getRootPath(obj, part)
                if rootObj is None:
                    continue
                paths.append(path)
    ...
    def applyStep(self, move, com=App.Vector(), size=100):
        ...
        for sub in subs:
            ...
            if move.ViewObject:
                startPos = UtilsMbDFEM.getCenterOfBoundingBox([obj], [ref])

            if move.MoveType == "Radial":
                objCom, objSize = UtilsMbDFEM.getComAndSize(obj)
                init_vec = objCom - com
            ...
            if move.ViewObject:
                endPos = UtilsMbDFEM.getCenterOfBoundingBox([obj], [ref])
...
class ViewProviderExplodedViewStep:
    ...
    def attach(self, vobj):
        ...
        self.so_color.rgb.setValue(UtilsMbDFEM.color_from_unsigned(param_step_line_color))
...
class TaskMbDFEMCreateView(QtCore.QObject):
    def __init__(self, viewObj=None):
        super().__init__()

        self.form = Gui.PySideUic.loadUi(":/panels/TaskMbDFEMCreateView.ui")
    ...
    def accept(self):
        ...
        commands = ""
        for move in self.viewObj.Group:
            more = UtilsMbDFEM.generatePropertySettings(move)
        ...
        for sel in selection:
            ...
            for sub_name in sel.SubElementNames:
                ...
                if partAsSolid:
                    ...
                    ref[1][0] = UtilsMbDFEM.truncateSubAtLast(ref[1][0], obj.Name)
                else:
                    ref[1][0] = UtilsMbDFEM.truncateSubAtFirst(ref[1][0], obj.Name)
    ...
    def onAlignTo(self):
        ...
        # clears selection before trying to select the whole MbDFEM
    ...
    def findDraggerInitialPlc(self):
        ...
        if self.alignMode == "Custom":
            ...
            if self.alignMode == "Center":
                self.initialDraggerPlc.Base = UtilsMbDFEM.getCenterOfBoundingBox(
    ...
    def createExplodedStepObject(self):
        ...
        # objects are within MbDFEM. So we put all the sub in a single ref.
        listOfSubs = []
    ...
    def moveMouse(self, info):
        ...
        plc = UtilsMbDFEM.findPlacement(ref)
        global_plc = UtilsMbDFEM.getGlobalPlacement(ref)
        plc = global_plc * plc
...
if App.GuiUp:
    Gui.addCommand("MbDFEM_CreateView", CommandCreateView())
