...
__title__ = "MbDFEM Command Create Simulation"
__author__ = "Ondsel"
...
class CommandCreateSimulation:
    ...
    def GetResources(self):
        return {
            "Pixmap": "MbDFEM_CreateSimulation",
            "MenuText": QT_TRANSLATE_NOOP("MbDFEM_CreateSimulation", "Simulation"),
            "Accel": "V",
            "ToolTip": QT_TRANSLATE_NOOP(
                "MbDFEM_CreateSimulation",
                "Creates a new simulation of the current MbDFEM",
            ),
            "CmdType": "ForEdit",
        }
...
class ViewProviderSimulation:
    ...
    def getIcon(self):
        return ":/icons/MbDFEM_CreateSimulation.svg"
...
class Motion:
    ...
    def getMbDFEM(self, feaPy):
        ...
        if simulation is not None:
            return simulation.Proxy.getMbDFEM(simulation)
        return None
...
class ViewProviderMotion:
    ...
    def updateLabel(self):
        ...
        self.app_obj.Label = "{label} ({type_})".format(
            label=self.app_obj.Joint[0].Label, type_=translate("MbDFEM", typeStr)
        )
...
class MotionEditDialog:
    def __init__(
        ...
        formula_edit.setPlaceholderText(translate("MbDFEM", "Enter your formula…"))
    ...
    def setupHelpSection(self):
        ...
        self.help_label1 = QLabel(translate("MbDFEM", " - Linear: C + VEL*time"), self.dialog)
        self.help_label2 = QLabel(
            translate("MbDFEM", " - Quadratic: C + VEL*time + ACC*time^2"), self.dialog
        )
        self.help_label3 = QLabel(
            translate("MbDFEM", " - Harmonic: C + AMP*sin(VEL*time - PHASE)"), self.dialog
        )
        self.help_label4 = QLabel(
            translate("MbDFEM", " - Exponential: C*exp(time/TIMEC)"), self.dialog
        )
...
SLOPE defines the steepness of the transition between 0 and H1 and H2 to 0 about time = T1 and T2 respectively. Higher values gives sharper cornered impulses. SLOPE = 1000 or greater are suitable.""",
        ...
        self.help_button = QPushButton(translate("MbDFEM", "Help"), self.dialog)
        ...
        def toggle_help():
            ...
            if show:
                self.help_button.setText(translate("MbDFEM", "Hide help"))
            else:
                self.help_button.setText(translate("MbDFEM", "Help"))
...
class TaskMbDFEMCreateSimulation(QtCore.QObject):
    def __init__(self, simFeaturePy=None):
        ...
        self.form = Gui.PySideUic.loadUi(":/panels/TaskMbDFEMCreateSimulation.ui")
    ...
    def saveAnimation(self):
        ...
        if num_frames <= 1:
            QMessageBox.warning(
                self.form,
                translate("MbDFEM", "Animation"),
                translate("MbDFEM", "Not enough frames to create an animation."),
            )
            return
        ...
        file_path, selected_filter = QFileDialog.getSaveFileName(
            self.form,
            translate("MbDFEM", "Save Animation"),
            "",
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            progress = QProgressDialog(
                translate("MbDFEM", "Generating Frames…"),
                translate("MbDFEM", "Cancel"),
                0,
                num_frames,
                self.form,
            )
            ...
            try:
                ...
                progress.setLabelText(translate("MbDFEM", "Assembling animation…"))
                ...
                errMsg = (
                    translate("MbDFEM", "An error occurred while saving the animation")
                    + ": "
                    + str(e)
                )
    ...
    def create_gif(self, output_path, frame_files, fps):
        ...
        try:
            ...
            errMsg = translate(
                "MbDFEM", "Pillow (PIL) is not installed. It is required for GIF export."
            )
            QMessageBox.critical(self.form, "Error", errMsg)
            return False
    ...
    def create_video(self, output_path, frame_files, fps, size):
        ...
        try:
            ...
            errMsg = translate(
                "MbDFEM", "OpenCV is not installed. It is required for video export."
            )
            QMessageBox.critical(self.form, "Error", errMsg)
            return False
        ...
        if not video_writer.isOpened():
            errMsg = translate("MbDFEM", "Could not open video writer. Check codecs.")
            QMessageBox.critical(self.form, "Error", errMsg)
            return False
...
if App.GuiUp:
    Gui.addCommand("MbDFEM_CreateSimulation", CommandCreateSimulation())
