...
from MbDSoSwitchMarker import MbDSoSwitchMarker

translate = App.Qt.translate

TranslatedJointTypes = [
    translate("MbDFEM", "Fixed"),
    translate("MbDFEM", "Revolute"),
    translate("MbDFEM", "Cylindrical"),
    translate("MbDFEM", "Slider"),
    translate("MbDFEM", "Ball"),
    translate("MbDFEM", "Distance"),
    translate("MbDFEM", "Parallel"),
    translate("MbDFEM", "Perpendicular"),
    translate("MbDFEM", "Angle"),
    translate("MbDFEM", "RackPinion"),
    translate("MbDFEM", "Screw"),
    translate("MbDFEM", "Gears"),
    translate("MbDFEM", "Belt"),
]
...
class Joint:
    ...
    def migrationScript(self, joint):
        if hasattr(joint, "Object1") and isinstance(joint.Object1, str):
            ...
            obj1 = UtilsMbDFEM.getObjectInPart(objName, joint.Part1)
            el1 = joint.Element1
            vtx1 = joint.Vertex1
        ...
        if hasattr(joint, "Object2") and isinstance(joint.Object2, str):
            ...
            obj2 = UtilsMbDFEM.getObjectInPart(objName, joint.Part2)
            el2 = joint.Element2
            vtx2 = joint.Vertex2
        ...
        def processObject(object_attr, reference_attr, part_attr, connector_label, order):
            try:
                if hasattr(joint, object_attr):
                    ...
                    root_obj, path = UtilsMbDFEM.getRootPath(base_obj, part)
                    base_obj = root_obj
                    elt = path + elt
                    vtx = path + vtx
                ...
                App.Console.PrintWarning(
                    f"{translate('MbDFEM', 'MbDFEM joint')} '{getContext(joint)}' "
                    f"{translate('MbDFEM', 'has an invalid')} '{object_attr}' "
                    f"{translate('MbDFEM', 'or related attributes')}. {str(e)}\n"
                )
        ...
        def processReference(reference_attr):
            try:
                if hasattr(joint, reference_attr):
                    ...
                    sub1 = UtilsMbDFEM.fixBodyExtraFeatureInSub(doc_name, ref[1][0])
                    sub2 = UtilsMbDFEM.fixBodyExtraFeatureInSub(doc_name, ref[1][1])
                ...
                App.Console.PrintWarning(
                    f"{translate('MbDFEM', 'MbDFEM joint')} '{getContext(joint)}' "
                    f"{translate('MbDFEM', 'has an invalid')} '{reference_attr}' "
                    f"{translate('MbDFEM', 'or related attributes')}. {str(e)}\n"
                )
    ...
    def onChanged(self, joint, prop):
        ...
        if prop == "Offset1" or prop == "Offset2":
            ...
            isMbDFEM = self.getMbDFEM(joint).isDerivedFrom("MbDFEM::MbDAssembly")
            if isMbDFEM and not presolved:
                solveIfAllowed(self.getMbDFEM(joint))
            else:
                self.updateJCSPlacements(joint)
    ...
    def execute(self, joint):
        errStr = joint.Label + ": " + QT_TRANSLATE_NOOP("MbDFEM", "Broken link in: ")
        if (
    ...
    def findPlacement(self, joint, ref, index=0):
        ...
        plc = UtilsMbDFEM.findPlacement(ref, ignoreVertex)
    ...
    def matchJCS(self, joint, savePlc=True, reverse=False):
        ...
        moving_part_global_jcs = UtilsMbDFEM.getJcsGlobalPlc(moving_placement, moving_part_ref)
        fixed_part_global_jcs = UtilsMbDFEM.getJcsGlobalPlc(fixed_placement, fixed_part_ref)

        if not sameDir:
            moving_part_global_jcs = UtilsMbDFEM.flipPlacement(moving_part_global_jcs)
    ...
    def preventParallel(self, joint):
        ...
        if part2ConnectedByJoint:
            ...
            globalJcsPlc = UtilsMbDFEM.getJcsGlobalPlc(joint.Placement2, joint.Reference2)
            ...
            part2.Placement = UtilsMbDFEM.applyRotationToPlacementAlongAxis(
            ...
            # Get the global JCS placement to find a suitable rotation axis (its own X-axis)
            globalJcsPlc = UtilsMbDFEM.getJcsGlobalPlc(joint.Placement1, joint.Reference1)
            # Transform the local X-axis vector (1,0,0) into the global coordinate system
            rotation_axis = globalJcsPlc.Rotation.multVec(App.Vector(1, 0, 0))

            part1.Placement = UtilsMbDFEM.applyRotationToPlacementAlongAxis(
    ...
    def areJcsSameDir(self, joint):
        globalJcsPlc1 = UtilsMbDFEM.getJcsGlobalPlc(joint.Placement1, joint.Reference1)
        globalJcsPlc2 = UtilsMbDFEM.getJcsGlobalPlc(joint.Placement2, joint.Reference2)

        return UtilsMbDFEM.arePlacementSameDir(globalJcsPlc1, globalJcsPlc2)

    def areJcsZParallel(self, joint):
        globalJcsPlc1 = UtilsMbDFEM.getJcsGlobalPlc(joint.Placement1, joint.Reference1)
        globalJcsPlc2 = UtilsMbDFEM.getJcsGlobalPlc(joint.Placement2, joint.Reference2)

        return UtilsMbDFEM.arePlacementZParallel(globalJcsPlc1, globalJcsPlc2)
...
class ViewProviderJoint:
    ...
    def attach(self, vobj):
        ...
        self.switch_JCS1 = MbDSoSwitchMarker(vobj)
        self.switch_JCS2 = MbDSoSwitchMarker(vobj)
        self.switch_JCS_preview = MbDSoSwitchMarker(vobj)
    ...
    def getIcon(self):
        if self.app_obj.JointType == "Fixed":
            return ":/icons/MbDFEM_CreateJointFixed.svg"
        elif self.app_obj.JointType == "Revolute":
            return ":/icons/MbDFEM_CreateJointRevolute.svg"
        elif self.app_obj.JointType == "Cylindrical":
            return ":/icons/MbDFEM_CreateJointCylindrical.svg"
        elif self.app_obj.JointType == "Slider":
            return ":/icons/MbDFEM_CreateJointSlider.svg"
        elif self.app_obj.JointType == "Ball":
            return ":/icons/MbDFEM_CreateJointBall.svg"
        elif self.app_obj.JointType == "Distance":
            return ":/icons/MbDFEM_CreateJointDistance.svg"
        elif self.app_obj.JointType == "Parallel":
            return ":/icons/MbDFEM_CreateJointParallel.svg"
        elif self.app_obj.JointType == "Perpendicular":
            return ":/icons/MbDFEM_CreateJointPerpendicular.svg"
        elif self.app_obj.JointType == "Angle":
            return ":/icons/MbDFEM_CreateJointAngle.svg"
        elif self.app_obj.JointType == "RackPinion":
            return ":/icons/MbDFEM_CreateJointRackPinion.svg"
        elif self.app_obj.JointType == "Screw":
            return ":/icons/MbDFEM_CreateJointScrew.svg"
        elif self.app_obj.JointType == "Gears":
            return ":/icons/MbDFEM_CreateJointGears.svg"
        elif self.app_obj.JointType == "Belt":
            return ":/icons/MbDFEM_CreateJointPulleys.svg"

        return ":/icons/MbDFEM_CreateJoint.svg"
...
class ViewProviderGroundedJoint:
    ...
    def attach(self, vobj):
        ...
        lockpadColorInt = Preferences.preferences().GetUnsigned("MbDFEMConstraints", 0xCC333300)
        self.lockpadColor = coin.SoBaseColor()
        self.lockpadColor.rgb.setValue(UtilsMbDFEM.color_from_unsigned(lockpadColorInt))
    ...
    def getIcon(self):
        return ":/icons/MbDFEM_ToggleGrounded.svg"
...
class MakeJointSelGate:
    ...
    def allow(self, doc, obj, sub):
        ...
        ref = [obj, [sub]]
        sel_obj = UtilsMbDFEM.getObject(ref)

        if UtilsMbDFEM.isLink(sel_obj):
...
class TaskMbDFEMCreateJoint(QtCore.QObject):
    def __init__(self, jointTypeIndex, jointObj=None, subclass=False):
        ...
        self.jForm = Gui.PySideUic.loadUi(":/panels/TaskMbDFEMCreateJoint.ui", self.form)
        ...
        self.jForm.isolateType.addItems(
            [translate("MbDFEM", mode) for mode in self.isolate_modes]
        )
    ...
    def accept(self):
        if len(self.refs) != 2:
            App.Console.PrintWarning(
                translate("MbDFEM", "Select 2 elements from 2 separate parts")
            )
            return False
    ...
    def deactivate(self):
        ...
        UtilsMbDFEM.setJointsPickableState(self.doc, True)
    ...
    def handleInitialSelection(self):
        ...
        for sel in selection:
            ...
            if not sel.SubElementNames:
                # no subnames, so its a root MbDFEM itself that is selected.
    ...
    def adaptUi(self):
        ...
        if needDistance:
            if jType == "Distance":
                self.jForm.distanceLabel.setText(translate("MbDFEM", "Distance"))
            elif jType == "Gears" or jType == "Belt":
                self.jForm.distanceLabel.setText(translate("MbDFEM", "Radius 1"))
            elif jType == "Screw":
                self.jForm.distanceLabel.setText(translate("MbDFEM", "Thread pitch"))
            else:
                self.jForm.distanceLabel.setText(translate("MbDFEM", "Pitch radius"))
    ...
    def onOffset1Clicked(self):
        UtilsMbDFEM.openEditingPlacementDialog(self.joint, "Offset1")
        self.updateOffsetWidgets()

    def onOffset2Clicked(self):
        UtilsMbDFEM.openEditingPlacementDialog(self.joint, "Offset2")
        self.updateOffsetWidgets()
    ...
    def updateIsolation(self):
        ...
        if len(self.refs) == 2:
            try:
                ...
                MbDFEM_vobj.isolateComponents(list(parts_to_isolate), isolate_mode)
            except Exception as e:
    ...
    def updateTaskboxFromJoint(self):
        ...
        if UtilsMbDFEM.isRefValid(ref1, 2):
            self.refs.append(ref1)
            sub1 = UtilsMbDFEM.addTipNameToSub(ref1)
            Gui.Selection.addSelection(ref1[0].Document.Name, ref1[0].Name, sub1)

        if UtilsMbDFEM.isRefValid(ref2, 2):
            self.refs.append(ref2)
            sub2 = UtilsMbDFEM.addTipNameToSub(ref2)
    ...
    def updateJointList(self):
        ...
        for ref in self.refs:
            ...
            sname = UtilsMbDFEM.getObject(ref).Label

            element_name = UtilsMbDFEM.getElementName(ref[1][0])
            if element_name != "":
    ...
    def updateLimits(self):
        ...
        if needLengthLimits:
            distance = UtilsMbDFEM.getJointDistance(self.joint)
            if (
        ...
        if needAngleLimits:
            angle = UtilsMbDFEM.getJointXYAngle(self.joint) / math.pi * 180
            if (
    ...
    def moveMouse(self, info):
        ...
        vertex_name = UtilsMbDFEM.findElementClosestVertex(ref, newPos)
    ...
    def _removeSelectedItems(self, selected_indexes):
        for index in selected_indexes:
            ...
            if row < len(self.refs):
                ref = self.refs[row]
                sub = UtilsMbDFEM.addTipNameToSub(ref)
    ...
    def getMovingPart(self, ref):
        return UtilsMbDFEM.getMovingPart(ref)
    ...
    def addSelection(self, doc_name, obj_name, sub_name, mousePos):
        ...
        vertex_name = UtilsMbDFEM.findElementClosestVertex(ref, mousePos)
